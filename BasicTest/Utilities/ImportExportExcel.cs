...
namespace BasicTest
{
    public class ImportExportExcel
    {
        ...
        /// <summary>
        /// Chuyển đổi data từ excel thành 1 Datatable (Bảng dữ liệu)
        /// </summary>
        /// <param name="FileName">Đường dẫn đến file excel</param>
        ...
        public static DataSet ConvertExcelToDataTable(string FileName)
        {
            using (OleDbConnection objConn = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + FileName + ";Extended Properties='Excel 12.0;HDR=YES;IMEX=1;';"))
            {
                ...
                {
                    sheetName = Convert.ToString(dr["TABLE_NAME"]);
                    OleDbCommand cmd = new OleDbCommand();
                    cmd.Connection = objConn;
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "SELECT * FROM [" + sheetName + "]";
                    oleda = new OleDbDataAdapter(cmd);
                    oleda.Fill(ds, sheetName.Replace("\'", "").Replace("$",""));
                }
                objConn.Close();
                return ds; //Returning Dattable  
            }
        }

        public static DataTable GetDataBySheetName(string FileName, string sheetName)
        {
            using (OleDbConnection objConn = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + FileName + ";Extended Properties='Excel 12.0;HDR=YES;IMEX=1;';"))
            {
                objConn.Open();
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    string currentSheetName = Convert.ToString(dr["TABLE_NAME"]);
                    if (!currentSheetName.Contains(sheetName)) continue;
                    OleDbCommand cmd = new OleDbCommand();
                    cmd.Connection = objConn;
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "SELECT * FROM [" + currentSheetName + "]";
                    oleda = new OleDbDataAdapter(cmd);
                    oleda.Fill(ds, sheetName);
                    result = ds.Tables[sheetName];
                }
                objConn.Close();
                return result; //Returning Dattable  
            }
        }
        #endregion

        #region DataSet/DataTable to Excel
